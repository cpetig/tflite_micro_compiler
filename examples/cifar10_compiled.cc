...
void cifar_invoke(void const* (inputs[1]), void * (outputs[1])) {
  ...
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::pooling::MaxEval(&cifar_context, &cifar_nodes[1]); // Input 12, Output 13,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::conv::Eval(&cifar_context, &cifar_nodes[2]); // Input 13,10,2, Output 14,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::pooling::MaxEval(&cifar_context, &cifar_nodes[3]); // Input 14, Output 15,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::conv::Eval(&cifar_context, &cifar_nodes[4]); // Input 15,11,3, Output 16,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::reshape::Eval(&cifar_context, &cifar_nodes[5]); // Input 16,6, Output 17,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::fully_connected::Eval(&cifar_context, &cifar_nodes[6]); // Input 17,7,4, Output 18,
  assert(status==kTfLiteOk);
  status = tflite::ops::micro::fully_connected::Eval(&cifar_context, &cifar_nodes[7]); // Input 18,8,5, Output 19,
  assert(status==kTfLiteOk);
}
