...
extern TfLiteStatus FullyConnectedEval(TfLiteContext* context,
                                       TfLiteNode* node);
namespace tflite {
namespace ops {
namespace micro {
...
static std::string to_string(TfLitePaddingValues const& v) {
  return std::string("{ ") + std::to_string(v.width) + "," +
         std::to_string(v.height) + ", " + std::to_string(v.width_offset) +
         "," + std::to_string(v.height_offset) + " }";
}

// implementation from
// https://github.com/tum-ei-eda/tflm-offline-interpreter/blob/master/src/main.cpp
...
static void dump_builtin(tflite::BuiltinOperator op, void const* data,
...
#define FUNCNAME(X) \
  if (ptr == &X) return std::make_pair<std::string, bool>(#X, true)
#define FUNC_SET(X)                         \
  FUNCNAME(tflite::ops::micro::X::Init);    \
  FUNCNAME(tflite::ops::micro::X::Prepare); \
  FUNCNAME(tflite::ops::micro::X::Eval)
#define FUNC_SET2(X)                        \
  FUNCNAME(tflite::ops::micro::X::Prepare); \
  FUNCNAME(tflite::ops::micro::X::Eval)
#define FUNC_SET2P(X, Y)                       \
  FUNCNAME(tflite::ops::micro::X::Y##Prepare); \
  FUNCNAME(tflite::ops::micro::X::Y##Eval)
...
// outside declarations are written later and init_statements inside the init
// function
static void declare_function(const void* ptr, tflite::BuiltinOperator op,
                             ...
                             std::string& outside_declarations,
  ...
  if (ptr) {
    ...
    if (!name.second) {
      if (known2.find(op) == known2.end()) {
        ...
        std::cout << "extern TfLiteRegistration *Register_" << opname
                  << "(void);\n";

        outside_declarations +=
            "static TfLiteRegistration *operator_" + opname + ";\n";
      ...
      if (name.first.substr(0, 20) == "tflite::ops::micro::") {
        ...
        if (sep != std::string::npos) {
          ...
          std::cout << "namespace " << nmspc << " { extern " << result << ' '
                    << fun << '(' << arguments << "); }" << '\n';
        }
      } else {  // hopefully outside of namespace declaration fits here
        outside_declarations += "extern " + std::string(result) + ' ' +
...
static void dump_int_array_real(TfLiteIntArray const& arr) {
  if (arr.size > 0) {
    std::cout << arr.data[0];
    for (uint32_t i = 1; i < arr.size; ++i) std::cout << ',' << arr.data[i];
  }
}
...
static void dump_tensor_contents(char const* prefix, TfLiteTensor const& t,
                                 char const* tname, uint32_t tensor_number) {
  if (t.dims->size == 0) {  // special case 0 dimensions, we output an array to
                            // avoid distinction from >0 dimension at every use
    std::cout << "static const " << tname << " " << prefix << "tensor_data"
              << tensor_number << "[1] = { "
              << (printT)(tflite::GetTensorData<T>(&t)[0]) << " };\n";
    return;
  }
  std::cout << "static const " << tname << " " << prefix << "tensor_data"
            << tensor_number << "[" << t.dims->data[0];
  for (uint32_t i = 1; i < t.dims->size; ++i)
  ...
  } else if (t.dims->size == 2)  // two dimensions: Inner dimension is one line
  {
    for (uint32_t i = 0; i < t.dims->data[0]; ++i) {
      ...
      for (uint32_t j = 0; j < t.dims->data[1]; ++j)
        std::cout << (printT)(
                         tflite::GetTensorData<T>(&t)[i * t.dims->data[1] + j])
                  << ", ";
    }
    std::cout << "\n};\n";
    ...
    for (uint32_t i = 0; i < outer_dim; ++i) {
      ...
      for (uint32_t j = 0; j < middle_dim; ++j) {
        for (uint32_t k = 0; k < inner_dim; ++k)
          std::cout << (printT)(tflite::GetTensorData<T>(
...
#define DUMP_TENSOR2(TfType, CType, PrintType)                                \
  case TfType:                                                                \
    dump_tensor_contents<CType, PrintType>(prefix, t, #CType, tensor_number); \
    break
static void dump_tensor(char const* prefix, TfLiteTensor const& t,
                        uint32_t tensor_number) {
  switch (t.type) {
    ...
    default: {
      std::cout << "static const uint8_t " << prefix << "tensor_data"
                << tensor_number << "[" << t.bytes << "] = { ";
...
static void dump_int_array(char const* prefix, char const* name,
                           uint32_t tensor_number, TfLiteIntArray const& arr) {
  std::cout << "static const int " << prefix << name << tensor_number << "["
            << (arr.size + 1) << "] = { " << arr.size << ",  ";
  dump_int_array_real(arr);
  std::cout << " };\n";
}

static void dump_dimension(char const* prefix, TfLiteTensor const& t,
                           uint32_t tensor_number) {
...
void dump_data(char const* prefix, tflite::MicroInterpreter* interpreter,
  ...
  tflite::Model const* model =
      ::tflite::GetModel(tflite_array);  // needed for size calculation
  ...
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
    declare_function(
        (const void*)(interpreter->node_and_registration(i).registration->init),
        tflite::BuiltinOperator(
            interpreter->node_and_registration(i).registration->builtin_code),
        known, known2, "void*", "TfLiteContext*, const char*, size_t",
        outside_declarations, init_statements);
    declare_function(
        (const void*)(interpreter->node_and_registration(i)
                          .registration->prepare),
        tflite::BuiltinOperator(
            interpreter->node_and_registration(i).registration->builtin_code),
        known, known2, "TfLiteStatus", "TfLiteContext*, TfLiteNode*",
        outside_declarations, init_statements);
    declare_function(
        (const void*)(interpreter->node_and_registration(i)
                          .registration->invoke),
  ...
  std::cout << "static TfLiteTensor " << prefix << "tensors["
            << interpreter->tensors_size() << "];" << '\n';
  std::cout << "static TfLiteNode " << prefix << "nodes["
            << interpreter->operators_size() << "];" << '\n';
  std::cout << "static TfLiteContext " << prefix << "context;" << '\n';
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
#ifndef EMBED_TENSORS  // for embed tensors this is unconditional
    if (mem_in(interpreter->node_and_registration(i).node.builtin_data,
               tensor_arena, arena_end))
#endif
    {
      dump_builtin(
          tflite::BuiltinOperator(
...
#ifdef EMBED_TENSORS
    dump_int_array(prefix, "inputs", i,
                   *interpreter->node_and_registration(i).node.inputs);
    dump_int_array(prefix, "outputs", i,
                   *interpreter->node_and_registration(i).node.outputs);
#endif
  }
  std::cout << '\n';
...
#ifdef EMBED_TENSORS
    if (t->allocation_type == kTfLiteMmapRo) dump_tensor(prefix, *t, i);
    dump_dimension(prefix, *t, i);
#endif
    if (t->quantization.type == kTfLiteAffineQuantization) {
      TfLiteAffineQuantization const* q =
          (TfLiteAffineQuantization const*)t->quantization.params;
      ...
      std::cout << "static const int " << prefix << "quant_zero" << i << "["
                << q->zero_point->size + 1 << "] = { " << q->zero_point->size
                << ", ";
      dump_int_array_real(*q->zero_point);
      std::cout << "};" << '\n';
      std::cout << "static const TfLiteAffineQuantization " << prefix
                << "quantization" << i << " = { "
  ...
  std::cout << "static TfLiteStatus AllocatePersistentBuffer(struct "
               "TfLiteContext* ctx, size_t bytes, void** ptr) {"
            << '\n';
...
#ifndef EMBED_TENSORS  // this argument is only needed if the tensors aren't
                       // dumped into the generated file
  ...
  for (uint32_t i = 0; i < interpreter->tensors_size(); ++i) {
    ...
    std::cout << "  " << prefix << "tensors[" << i
              << "].allocation_type = " << to_string(t->allocation_type) << ';'
              << '\n';
    ...
    if (mem_in(t->name, tflite_array, tflite_end)) {
      std::cout << "  " << prefix << "tensors[" << i
                << "].name = (char*)(tflite_array + "
                << (((uint8_t const*)t->name) - tflite_array) << "); /* "
                << t->name << " */" << '\n';
    } else
#endif
    {
      std::cout << "  " << prefix << "tensors[" << i << "].name = (char*)\""
                << t->name << "\";" << '\n';
    }
    if (mem_in(t->dims, tflite_array, tflite_end)) {
#ifdef EMBED_TENSORS
      std::cout << "  " << prefix << "tensors[" << i
                << "].dims = (struct TfLiteIntArray*)" << prefix
                << "tensor_dimension" << i << ";\n";
#else
      std::cout << "  " << prefix << "tensors[" << i
                << "].dims = (struct TfLiteIntArray*)(tflite_array + "
                << (((uint8_t const*)t->dims) - tflite_array) << "); /* (";
...
#ifdef EMBED_TENSORS
      std::cout << "  " << prefix << "tensors[" << i
                << "].data.raw_const = (const char*)" << prefix << "tensor_data"
                << i << ";\n";
#else
      std::cout << "  " << prefix << "tensors[" << i
                << "].data.raw_const = (const char*)(tflite_array + "
                << (((uint8_t const*)t->data.raw_const) - tflite_array) << ");"
                << '\n';
#endif
    else if (mem_in(t->data.raw_const, tensor_arena, arena_end))
      std::cout << "  " << prefix << "tensors[" << i
                << "].data.raw = (char*)(tensor_arena + "
                << (((uint8_t const*)t->data.raw_const) - tensor_arena) << ");"
                << '\n';
    if (t->params.scale != 0.0f) {
      std::cout << "  " << prefix << "tensors[" << i
                << "].params.scale = " << t->params.scale << ";" << '\n';
      std::cout << "  " << prefix << "tensors[" << i
                << "].params.zero_point = " << t->params.zero_point << ";"
                << '\n';
    }
    if (t->quantization.type == kTfLiteAffineQuantization) {
      std::cout << "  " << prefix << "tensors[" << i
      ...
      std::cout << "  " << prefix << "tensors[" << i
                << "].quantization.params = (void*)&" << prefix
                << "quantization" << i << ";" << '\n';
    }
  }
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
#ifdef EMBED_TENSORS
    std::cout << "  " << prefix << "nodes[" << i
              << "].inputs = (struct TfLiteIntArray*)" << prefix << "inputs"
              << i << ";\n";
    std::cout << "  " << prefix << "nodes[" << i
              << "].outputs = (struct TfLiteIntArray*)" << prefix << "outputs"
              << i << ";\n";
#else
    if (mem_in(interpreter->node_and_registration(i).node.inputs, tflite_array,
               tflite_end)) {
      std::cout << "  " << prefix << "nodes[" << i
                << "].inputs = (struct TfLiteIntArray*)(tflite_array + "
                << (((uint8_t const*)interpreter->node_and_registration(i)
                         .node.inputs) -
                    tflite_array)
                << "); /* (";
    ...
    if (mem_in(interpreter->node_and_registration(i).node.outputs, tflite_array,
               tflite_end)) {
      std::cout << "  " << prefix << "nodes[" << i
                << "].outputs = (struct TfLiteIntArray*)(tflite_array + "
                << (((uint8_t const*)interpreter->node_and_registration(i)
                         .node.outputs) -
                    tflite_array)
                << "); /* (";
    ...
    {
      std::cout << "  " << prefix << "nodes[" << i
                << "].builtin_data = (void*)&" << prefix << "opdata" << i << ";"
                << '\n';
    }
  }
  std::cout << "  " << prefix
            << "context.tensors_size = " << interpreter->tensors_size() << ";"
            << '\n';
  std::cout << "  " << prefix << "context.tensors = (TfLiteTensor*)" << prefix
            << "tensors;" << '\n';
  std::cout << "  " << prefix
            << "context.AllocatePersistentBuffer = &AllocatePersistentBuffer;"
            << '\n';
  // this code assumes that persistent allocations are made from the end (which
  // is true for the current implementation)
  std::cout << "  "
            << "next_allocation = tensor_arena + " << (arena_end - tensor_arena)
            << "; // = minimum size of the tensor arena\n";
  std::cout << "  "
            << "TfLiteStatus status = kTfLiteOk;\n";
  std::cout << init_statements;
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
    if (interpreter->node_and_registration(i).registration->init) {
      // TODO: There is a good chance that just assigning user_data will do the
      // trick as well (unless it gets initialized)
      std::pair<std::string, bool> name =
          function_name((const void*)(interpreter->node_and_registration(i)
                                          .registration->init));
      if (!name.second)
        name.first =
            "(*(operator_" +
            std::string(tflite::EnumNameBuiltinOperator(
                tflite::BuiltinOperator(interpreter->node_and_registration(i)
                                            .registration->builtin_code))) +
            "->init))";
      std::cout << "  " << prefix << "nodes[" << i << "].user_data = "
                << name.first
                // TODO: Handle custom operators
                << "(&" << prefix << "context, (const char*)(" << prefix
                << "nodes[" << i << "].builtin_data), 0);" << '\n';
    }
  }
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
        ...
        name.first =
            ...
            std::string(tflite::EnumNameBuiltinOperator(
                tflite::BuiltinOperator(interpreter->node_and_registration(i)
                                            .registration->builtin_code))) +
            "->prepare))";
      std::cout << "  status = " << name.first << "(&" << prefix << "context, &"
                << prefix << "nodes[" << i << "]);" << '\n';
  ...
  //   figure out whether the input/output types are homogenic (then use a
  ...
  // some more magic to make the generated code more readable (with respect to
  // the arguments to invoke)
  if (interpreter->inputs().size() == 1) {
    if ((interpreter->input(0)->dims->size == 0 ||
         (interpreter->input(0)->dims->size == 1 &&
    ...
    std::cout << c_type(intype) << " const* (inputs["
  ...
  if (interpreter->outputs().size() == 1) {
    ...
    std::cout << c_type(outtype) << " * (outputs["
              << interpreter->outputs().size() << "])";
  }
  std::cout << ") {" << '\n';
  if (interpreter->inputs().size() == 1) {
    std::cout << "  " << prefix << "tensors[" << interpreter->inputs()[0]
              << "].data.raw_const = (const char*)";
    if ((interpreter->input(0)->dims->size == 0 ||
         (interpreter->input(0)->dims->size == 1 &&
          interpreter->input(0)->dims->data[0] == 1)) &&
        c_type(intype) != "void")
    ...
    for (uint32_t i = 0; i < interpreter->inputs().size(); ++i) {
      std::cout << "  " << prefix << "tensors[" << interpreter->inputs()[i]
                << "].data.raw_const = (const char*)(inputs[" << i
                << "]); // (";
  ...
  if (interpreter->outputs().size() == 1) {
    std::cout << "  " << prefix << "tensors[" << interpreter->outputs()[0]
              << "].data.raw = (char*)output; // (";
    dump_int_array_real(*(interpreter->output(0)->dims));
    std::cout << ")\n";
  } else {
    for (uint32_t i = 0; i < interpreter->outputs().size(); ++i) {
      std::cout << "  " << prefix << "tensors[" << interpreter->outputs()[i]
                << "].data.raw = (char*)(outputs[" << i << "]); // (";
  ...
  std::cout << "  "
            << "TfLiteStatus status = kTfLiteOk;\n";
  for (uint32_t i = 0; i < interpreter->operators_size(); ++i) {
    std::pair<std::string, bool> funname =
        function_name((const void*)(interpreter->node_and_registration(i)
                                        .registration->invoke));
    if (!funname.second)
      funname.first =
          ...
          std::string(tflite::EnumNameBuiltinOperator(
              tflite::BuiltinOperator(interpreter->node_and_registration(i)
                                          .registration->builtin_code))) +
          "->invoke))";
    std::cout << "  status = " << funname.first << "(&" << prefix
              << "context, &" << prefix << "nodes[" << i << "]); // Input ";
    dump_int_array_real(*interpreter->node_and_registration(i).node.inputs);
    std::cout << " Output ";
    dump_int_array_real(*interpreter->node_and_registration(i).node.outputs);
    std::cout << "\n";
