...
enum AllocKind : int { Persistent, Scratch };
...
// API calls.  Painful to maintain and prone subtle Bugs.  Simpler to maintain a
// patch that adds hooks to MicroInterpreter to gather data from an actual
// MicroInterpreter::AllocateTensors() by intercepting the TfliteContext vectors
...
void SetRecordAllocationhooks(tflite::MicroInterpreter *interpreter,
                              uint8_t *arena_start, size_t arena_size);

void RecordScratchBufferAllocations(tflite::MicroInterpreter *interpreter);

#else
void RecordAllocations(const tflite::Model *model, size_t arena_size,
